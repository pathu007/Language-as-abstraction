...
from clevr_robot_env.third_party.clevr_robot_env_utils.generate_question import generate_question_from_scene_struct
  ...
  def __init__(self,
    ...
    if self.direct_obs:
      self.observation_space = spaces.Box(
          low=np.concatenate(([-0.6] * num_object, [-0.4] * num_object)),
          high=np.concatenate(([0.6] * num_object, [0.6] * num_object)),
          dtype=np.float32)
    else:
      self.observation_space = spaces.Box(
